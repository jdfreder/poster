...
def convert_regex(regex):
    try:
        output = subprocess.check_output('node vim_regex.js "{}"'.format(regex.replace('\\', '\\\\').replace('"', '\\"')), shell=True)
...
def parse_regex(regex):
    ...
    if regex.endswith('me=e-1'):
        ...
        delta = 1
        regex = regex[:-6]
    conversion = convert_regex(regex)
    if conversion is None:
        return {'regex': 'ERROR!ERROR!ERROR!', 'flags': '', 'delta': 0}
    else:
        return {'regex': conversion['regex'], 'flags': conversion['flags'], 'delta': delta}
